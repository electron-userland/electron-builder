...
var LinuxBuilder = {
  ...
  build : function( options, callback ) {
    ...
    var tmpFolder = path.join( os.tmpDir(), linux.exec );
...
function _createDesktopEntry( tmpFolder, linux ) {
  ...
  var dest = path.join(tmpFolder, 'usr', 'share', 'applications', linux.exec + '.desktop');
...
function _copyApp( options, tmpFolder ) {
  fs.copySync(
    ...
    path.join( tmpFolder, 'opt', options.config.linux.exec )
  );
}
...
function _buildPackage( options, scripts, tmpFolder ) {
  var file = options.config.linux.exec + '-VERSION-ARCH.' + options.config.linux.target;
  var dest = path.join(options.out, file);
  var args = [
    '-s dir',
    '-t ' + options.config.linux.target,
    '--architecture ' + options.config.linux.archName,
    '--rpm-os linux',
    '--name "' + options.config.linux.title + '"',
    '--force',
    '--after-install ' + scripts[0],
    '--after-remove ' + scripts[1],
    '--description "' + options.config.linux.comment + '"',
    '--package "' + dest + '"',
    '-C ' + tmpFolder,
    '.'
  ];

  fs.writeFileSync(path.join(tmpFolder, 'opt', options.config.linux.exec, 'pkgtarget'), options.config.linux.target, 'utf8');
