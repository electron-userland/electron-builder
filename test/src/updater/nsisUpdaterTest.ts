...
test("github allowPrerelease=true", async ({ expect }) => {
...
test("github allowPrerelease=false", async ({ expect }) => {
  const updater = await createNsisUpdater("1.0.1")
...
test("file url generic", async ({ expect }) => {
...
test("file url generic - manual download", async ({ expect }) => {
  const updater = await createNsisUpdater()
  updater.updateConfigPath = await writeUpdateConfig<GenericServerOptions>({
    provider: "generic",
    url: "https://develar.s3.amazonaws.com/test",
...
test("checkForUpdates several times", async ({ expect }) => {
  const updater = await createNsisUpdater()
  updater.updateConfigPath = await writeUpdateConfig<GenericServerOptions>({
    provider: "generic",
    url: "https://develar.s3.amazonaws.com/test",
...
test.ifEnv(process.env.GH_TOKEN || process.env.GITHUB_TOKEN)("file url github private", async ({ expect }) => {
...
test.skip("test download progress", async ({ expect }) => {
  const updater = await createNsisUpdater("0.0.1")
...
test("valid signature", async ({ expect }) => {
  const updater = await createNsisUpdater("0.0.1")
  updater.updateConfigPath = await writeUpdateConfig({
    provider: "github",
    owner: "develar",
...
test.ifWindows("test custom signature verifier", async ({ expect }) => {
...
// disable for now
test("90 staging percentage", async ({ expect }) => {
...
test("test download and install", async ({ expect }) => {
  const updater = await createNsisUpdater()
  updater.updateConfigPath = await writeUpdateConfig<GenericServerOptions>({
    provider: "generic",
    url: "https://develar.s3.amazonaws.com/test",
