import { DataSplitter, PartListDataTask } from "electron-updater/out/differentialDownloader/DataSplitter"
import { Operation, OperationKind } from "electron-updater/out/differentialDownloader/downloadPlanBuilder"
import { executeTasksUsingMultipleRangeRequests } from "electron-updater/out/differentialDownloader/multipleRangeDownloader"
...
describe("differential multipart download", () => {
  ...
  // release/v26 has no per-test `tmpDir` context fixture (added to master's vitest-setup), so manage one here.
  const tmpDir = new TmpDir("differential-multipart-test")
  afterEach(() => tmpDir.cleanup())

  // Writes `content` into the suite's temp dir and opens it read-only; the caller is responsible for closing the fd.
  ...
  for (const { name, options } of cases) {
    test(`splits every part correctly: ${name}, any chunking`, async ({ expect }) => {
  ...
  describe("multiple batches", () => {
    ...
    test("a finished batch does not fail a slower next batch", async ({ expect }) => {
