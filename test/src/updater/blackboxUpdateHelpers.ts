...
import { archFromString, getArchSuffix, log, spawn, TmpDir } from "builder-util/out/util"
...
export async function handleInitialInstallPerOS({ target, dirPath, arch }: { target: string; dirPath: string; arch: Arch }): Promise<string> {
  let appPath: string
  if (target === "AppImage") {
    ...
    DebUpdater.installWithCommandRunner(
      ...
      commandWithArgs => {
        execFileSync(commandWithArgs.join(" "), { stdio: "inherit" })
      },
      console
    )
...
export async function runTestWithinServer(doTest: (rootDirectory: string, updateConfigPath: string) => Promise<void>) {
