...
/**
 * Register mac blackbox auto-update cases. Every case is a full build → install → launch → update cycle (6-16 min
 * each on the macOS CI runner), and the CI sharder bin-packs whole FILES, so each case lives in its own
 * `test/src/updater/mac/blackboxUpdate*Test.ts` and passes its name here; that lets the sharder place the cases on
 * different shards while the describe path — and thus every snapshot key (`mac > <case> N`) — stays identical to
