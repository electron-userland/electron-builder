...
describe("clearCache", () => {
  ...
  test("deletes cache dir when it exists and user confirms", async () => {
    ...
    expect(log.info.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ cacheDir: expect.any(String) }), "cache cleared")
  })
  ...
  test("does not delete and logs when cache dir does not exist", async () => {
    ...
    expect(log.info.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ cacheDir: expect.any(String) }), "cache directory does not exist, nothing to clear")
  })
  ...
  test("does not delete when cache dir is not writable and logs error", async () => {
    ...
    await clearCache()
    expect(rm).not.toHaveBeenCalled()
    expect(log.error.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ cacheDir: expect.any(String) }), "cache directory is not writable")
  })
  ...
  test("aborts and logs error when cache dir resolves to filesystem root", async () => {
    ...
    await clearCache()
    expect(rm).not.toHaveBeenCalled()
    expect(log.error.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ cacheDir: "/" }), expect.stringContaining("filesystem root"))
  })
  ...
  for (const answer of ["n", "N", "no", "", "   ", "maybe"]) {
    test(`aborts when user answers "${answer}"`, async () => {
      ...
      await clearCache()
      expect(rm).not.toHaveBeenCalled()
      expect(log.info.bind(log)).toHaveBeenCalledWith(null, "aborted")
    })
  }
})
...
describe("wrap", () => {
  ...
  test("sets exitCode=1 and logs message only for InvalidConfigurationError", async () => {
    ...
    expect(log.error.bind(log)).toHaveBeenCalledWith(null, "bad config")
  })
  ...
  test("sets exitCode=1 but does not re-log ExecError when alreadyLogged=true", async () => {
    ...
    await wrap(task)({})
    expect(process.exitCode).toBe(1)
    expect(log.error.bind(log)).not.toHaveBeenCalled()
  })
  ...
  test("logs ExecError with stack trace when alreadyLogged=false", async () => {
    ...
    await wrap(task)({})
    expect(process.exitCode).toBe(1)
    expect(log.error.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ stackTrace: expect.any(String) }), expect.any(String))
  })
  ...
  test("logs stack trace for generic errors", async () => {
    ...
    await wrap(task)({})
    expect(process.exitCode).toBe(1)
    expect(log.error.bind(log)).toHaveBeenCalledWith(expect.objectContaining({ stackTrace: expect.any(String) }), "something went wrong")
  })
})
