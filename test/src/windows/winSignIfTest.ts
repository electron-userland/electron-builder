...
const baseConfig = {
  toolsets: { winCodeSign: "1.1.0" as const },
  win: {
    signtoolOptions: {
      certificateFile: "secretFile",
      certificatePassword: "pass",
      signingHashAlgorithms: ["sha256"] as ["sha256"],
    },
  },
}
...
describe("signIf", () => {
  describe("normal flow", () => {
    test("signs all files when no errors occur", async ({ expect }) => {
      ...
      const sign: CustomWindowsSign = async () => {
        callCount++
      }
      ...
      await app(expect, {
        ...
        config: {
          ...baseConfig,
          win: { signtoolOptions: { ...baseConfig.win.signtoolOptions, sign } },
        },
      })
    ...
    test("rejection propagates to caller even with forceCodeSigning enabled", async ({ expect }) => {
      ...
      await app(expect, {
        ...
        config: {
          ...baseConfig,
          win: {
            signtoolOptions: { ...baseConfig.win.signtoolOptions, sign },
