...
async function createDummyCert(tmpDir: TmpDir, name: string): Promise<string> {
...
describe("createKeychain security command wiring", { sequential: true }, () => {
  ...
  const tmpDir = new TmpDir("keychain-partition-list-test")
  ...
  test("set-key-partition-list receives the keychain password, not the certificate import password", async () => {
  ...
  test("every imported certificate gets its partition list set with the keychain password", async () => {
    const cscLink = await createDummyCert(tmpDir, "app")
