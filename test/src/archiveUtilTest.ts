...
describe("archive() storedPaths", { sequential: true }, () => {
  ...
  test("stored member is appended with Copy while the rest stays compressed", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    const outFile = path.join(tmpDir, "stored.7z")
  ...
  test("stored member's raw bytes appear verbatim in the archive", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    const outFile = path.join(tmpDir, "verbatim.7z")
  ...
  test("works with the exact differential-aware NSIS options (Copy is install-time decodable)", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    const outFile = path.join(tmpDir, "differential.7z")
  ...
  test("stored paths that do not exist are skipped", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    const outFile = path.join(tmpDir, "missing.7z")
  ...
  test("stored path with '..' throws", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    await expect(archive("7z", path.join(tmpDir, "traversal.7z"), dir, { withoutDir: true, storedPaths: ["../secret"] })).rejects.toThrow("path traversal sequence")
  })

  test("without withoutDir the stored member is prefixed with the directory name", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles) // lands at <tmp>/src
    const outFile = path.join(tmpDir, "prefixed.7z")
  ...
  test("append pass leaves the compressed pass's packed streams byte-identical", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    const storedOut = path.join(tmpDir, "with-stored.7z")
    ...
    const baselineDir = path.join(tmpDir, "baseline-src")
    ...
    const baselineOut = path.join(tmpDir, "baseline.7z")
  ...
  test("append pass mirrors isArchiveHeaderCompressed=false into the final header", async ({ expect }) => {
    const dir = await makeSrcDir(appFiles)
    ...
    const plainHeaderOut = path.join(tmpDir, "plain-header.7z")
    ...
    const defaultHeaderOut = path.join(tmpDir, "default-header.7z")
  ...
  test.ifNotWindows("append pass mirrors preserveSymlinks for a stored symlink member", async ({ expect }) => {
    const dir = await makeSrcDir({ "app.txt": appFiles["app.txt"], "resources/real.asar": appFiles["resources/app.asar"] })
    await fs.symlink("real.asar", path.join(dir, "resources", "app.asar"))

    const outFile = path.join(tmpDir, "stored-symlink.7z")
    await archive("7z", outFile, dir, { withoutDir: true, preserveSymlinks: true, storedPaths: ["resources/app.asar"] })

    const extractDir = path.join(tmpDir, "extracted-stored-symlink")
    await fs.mkdir(extractDir, { recursive: true })
    const { getPath7za } = await import("app-builder-lib/src/toolsets/7zip")
    const { exec: cpExec } = await import("child_process")
    const { promisify } = await import("util")
  ...
  test("ELECTRON_BUILDER_COMPRESSION_LEVEL does not reach stored members", async ({ expect }) => {
    vi.stubEnv("ELECTRON_BUILDER_COMPRESSION_LEVEL", "9")
    const dir = await makeSrcDir(appFiles)
    const outFile = path.join(tmpDir, "env-override.7z")
    await archive("7z", outFile, dir, { withoutDir: true, storedPaths: ["resources/app.asar"] })
