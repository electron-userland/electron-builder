...
test("loadUpdateSigningKeys: ELECTRON_BUILDER_UPDATE_SIGN_KEY may hold several concatenated PEM blocks", ({ expect }) => {
  const a = generateUpdateSigningKeypair()
  const b = generateUpdateSigningKeypair()
...
test("loadUpdateSigningKeys: the first configured SOURCE wins even when a later source has more keys", ({ expect }) => {
  const a = generateUpdateSigningKeypair()
  const b = generateUpdateSigningKeypair()
...
test("loadUpdateSigningKeys rejects duplicate keys and non-Ed25519 keys with a clear error", ({ expect }) => {
  const a = generateUpdateSigningKeypair()
  stubSigningEnv({})
...
function makeAppUpdateConfigPackager(signingKeys: Array<KeyObject> = [], updateManifest?: any): any {
  const config = { publish: { provider: "generic", url: "https://example.com/updates" } }
  const appInfo = { updaterCacheDirName: "test-app", channel: null, version: "1.0.0" }
  return {
    platform: Platform.LINUX,
    platformSpecificBuildOptions: updateManifest == null ? {} : { updateManifest },
    config,
    appInfo,
    // the publish configuration is resolved against the shared Packager (`info`) as well
    info: { config, appInfo },
