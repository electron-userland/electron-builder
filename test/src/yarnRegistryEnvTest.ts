...
afterEach(() => {
  delete process.env.XXX_NPM_TOKEN
  delete process.env.GITHUB_TOKEN
  vi.clearAllMocks()
})

test("preserves the environment variable referenced by a Yarn Berry npmAuthToken", async ({ expect }) => {
  const projectDir = await mkdtemp(path.join(tmpdir(), "electron-builder-yarn-registry-env-"))
  try {
...
test("does not preserve sensitive variables referenced outside Yarn npmAuthToken settings", async ({ expect }) => {
  const projectDir = await mkdtemp(path.join(tmpdir(), "electron-builder-yarn-registry-env-"))
  try {
    await writeFile(
...
test("passes only Yarn npmAuthToken variables to the Yarn Berry install child", async ({ expect }) => {
  const projectDir = await mkdtemp(path.join(tmpdir(), "electron-builder-yarn-registry-env-"))
  try {
    await writeFile(path.join(projectDir, ".yarnrc.yml"), ['npmAuthToken: "${XXX_NPM_TOKEN}"', 'npmRegistryServer: "${GITHUB_TOKEN}"'].join("\n"))
    process.env.XXX_NPM_TOKEN = "registry-token"
    process.env.GITHUB_TOKEN = "publish-token"
