...
async function createAppOutDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "electron-languages-test-"))
  tempDirs.push(dir)
  return dir
}
...
describe("removeUnusedLanguagesIfNeeded", () => {
  test("bare language keeps region-qualified locales", async ({ expect }) => {
    const appOutDir = await createAppOutDir()
  ...
  test("region-qualified language keeps only the exact locale", async ({ expect }) => {
    const appOutDir = await createAppOutDir()
  ...
  test("matching is case-insensitive and supports underscore separators", async ({ expect }) => {
    const appOutDir = await createAppOutDir()
  ...
  test("region-qualified language keeps mac's bare .lproj dir", async ({ expect }) => {
    const appOutDir = await createAppOutDir()
  ...
  test("refuses to delete every locale when nothing matches", async ({ expect }) => {
    const appOutDir = await createAppOutDir()
