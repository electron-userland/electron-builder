...
describe("PnpmNodeModulesCollector hoisted mode", { concurrent: false }, () => {
  let root = ""
  afterEach(async () => {
    if (root) {
...
describe("PnpmNodeModulesCollector.isHoisted (on-disk layout detection)", { concurrent: false }, () => {
  let root = ""
...
describe("nested dependency resolution (hoisted layout simulation)", { concurrent: false }, () => {
  let root = ""
  afterEach(async () => {
    if (root) {
      await fse.rm(root, { recursive: true, force: true })
...
describe("PnpmNodeModulesCollector hoisted workspace package", () => {
  ...
  test("finds the nested in-range copy under the workspace root when the app dir has no node_modules", async ({ expect, tmpDir }) => {
  ...
  test("still finds the nested copy in a non-workspace hoisted project", async ({ expect, tmpDir }) => {
    const root = await tmpDir.createTempDir()
