...
// Cross-platform wine suites — emitted by both linux and mac generators (idempotent, ifNotWindows chain).
const WINE_SUITES: SuiteConfig[] = [
  {
    name: "wineToolset",
...
function renderWineSuiteFile(suite: SuiteConfig, version: ToolsetConfig["wine"]): string {
  const fnName = suite.registerFn.name
...
${describeCall}("${suite.describeConfig.name}"${optionsPart}, () => {
  ${fnName}(${toolsetsArg})
})
`
}
...
export function generateLinuxToolsetTests(): void {
  ...
  // Wine suites — cross-platform (ifNotWindows), idempotent with mac generator.
  for (const suite of WINE_SUITES) {
    const generatedDir = path.resolve(GENERATED_TESTS_DIR, suite.name)
    cleanAndEnsureDir(generatedDir)
    const platformSuffix = getPlatformSuffix(suite.describeConfig.chain)
    for (const version of WINE_VERSIONS) {
      const filename = `${suite.name}__wine-${version}${platformSuffix}Test.ts`
      fs.writeFileSync(path.join(generatedDir, filename), renderWineSuiteFile(suite, version), "utf8")
    }
  }
}
