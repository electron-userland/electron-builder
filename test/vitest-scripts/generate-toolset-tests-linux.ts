...
function renderWineFile(version: ToolsetConfig["wine"]): string {
  const fnName = WINE_SUITE.registerFn.name
  const toolsetsArg = JSON.stringify({ wine: version })
  const describeCall = buildDescribeCall(WINE_SUITE.describeConfig.chain)
  const optionsPart = WINE_SUITE.describeOptions ? `, ${JSON.stringify(WINE_SUITE.describeOptions)}` : ""
  const generatedDir = path.resolve(GENERATED_TESTS_DIR, WINE_SUITE.name)
  const importPath = resolveImportPath(generatedDir, TEST_SRC_DIR, WINE_SUITE.importPath)
  return `// @generated — do not edit. Regenerated by generate-toolset-tests-linux.ts on each test run.
import { ${fnName} } from "${importPath}"

${describeCall}("${WINE_SUITE.describeConfig.name}"${optionsPart}, () => {
  ${fnName}(${toolsetsArg})
})
`
}
...
export function generateLinuxToolsetTests(): void {
  ...
  // Wine toolset — cross-platform (ifNotWindows), idempotent with mac generator.
  const wineGeneratedDir = path.resolve(GENERATED_TESTS_DIR, WINE_SUITE.name)
  cleanAndEnsureDir(wineGeneratedDir)
  const winePlatformSuffix = getPlatformSuffix(WINE_SUITE.describeConfig.chain)
  for (const version of WINE_VERSIONS) {
    const filename = `${WINE_SUITE.name}__wine-${version}${winePlatformSuffix}Test.ts`
    fs.writeFileSync(path.join(wineGeneratedDir, filename), renderWineFile(version), "utf8")
  }
}
