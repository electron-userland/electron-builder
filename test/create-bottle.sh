...
#curl -X DELETE -u${ACCOUNT}:${API_KEY} https://api.bintray.com/content/develar/bin/electron-builder-mac-test-bottle/1.0/wine.7z || true
curl --progress-bar -T ~/wine.7z -u${ACCOUNT}:${API_KEY} https://api.bintray.com/content/develar/bin/electron-builder-mac-test-bottle/1.0/wine.7z