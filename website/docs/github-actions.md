...
Configure `win.sign` with `type: "azure"` in your electron-builder config. See [Windows Code Signing](features/code-signing/code-signing-win.md#azure-trusted-signing-type-azure-beta).
...
: Windows CI environments may truncate environment variables over 8192 characters. Re-export the `.pfx` without the full certificate chain included. See [Code Signing](features/code-signing/code-signing.md#encoding-a-certificate-for-ci).
...
- [Code Signing](features/code-signing/code-signing.md) — environment variables, certificate types
- [macOS Notarization](notarization.md) — notarization setup and troubleshooting
- [Auto Update](features/auto-update.md) — configuring electron-updater with GitHub releases
- [Multi Platform Build](features/multi-platform-build.md) — building for other platforms using Docker
