...
- **`fpm`** has only one published version today (`2.2.1`), so `"latest"` and that version are equivalent. **`linuxToolsMac`** and **`sevenZip`** also accept the earlier `"1.0.0"` bundle as an explicit pin.
:::
