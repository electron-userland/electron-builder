...
| `electronUpdaterCompatibility` removed | ✓ | Auto-detected from `dependencies['electron-updater']`; legacy `update.yml` fields only emitted when min version `< 4.0.0` (with warning) |
| `UpdateCheckResult.versionInfo` (electron-updater) removed | — | Use `result.updateInfo` (the alias has pointed at `updateInfo` since electron-updater 4.0) |
| Bintray `BT_TOKEN` publish auto-detection removed | — | Bintray was sunset in 2021; pick a supported `publish` provider |
...
| `toolsets.winCodeSign: "1.3.0"` for Azure dlib | — | Required for `signtool /dlib` Azure Trusted Signing; set explicitly when using `type: "azure"` on v27+ |
...
| `PlatformPackager.info` and `platformSpecificBuildOptions` are now `protected` | — | Plugin authors: use the new public pass-through getters / methods |
...
### `electronUpdaterCompatibility` (auto-detected)

The root-level and per-platform (`mac` / `win` / `linux`) `electronUpdaterCompatibility` field is removed. The publisher now reads `dependencies["electron-updater"]` from your `package.json` and decides automatically whether to emit the legacy `update.yml` compatibility fields (`path`, `sha512`, `sha2`) and the `<channel>-mac.json` artifact:

| Detected `electron-updater` range | Behaviour |
|---|---|
| Absent | Modern `files[]` format only |
| Min version >= `4.0.0` | Modern `files[]` format only |
| Min version < `4.0.0` | Legacy fields written + a one-shot upgrade-recommendation warning at build time |
| Unparseable range | Treated as absent (modern format) |

```json5
// Before (v26)
{ "build": { "electronUpdaterCompatibility": ">=2.16" } }

// After (v27) — delete the line; the default is now safer (modern format unless
// an older electron-updater is detected). `migrate-schema` handles this for you.
```

If your app pins `electron-updater` to a 3.x or older line and you rely on the legacy `update.yml`, upgrade `electron-updater` to >= 4.0.0 — the legacy publish path will be removed in a future major release.

### `UpdateCheckResult.versionInfo`

The `versionInfo` alias on the `UpdateCheckResult` returned by `autoUpdater.checkForUpdates()` is removed. It has been an alias for `updateInfo` since electron-updater 4.0.

```ts
// Before
const result = await autoUpdater.checkForUpdates()
console.log(result.versionInfo.version)  // ← compile error in v27

// After
console.log(result.updateInfo.version)
```

### Bintray publish auto-detection

Bintray was sunset in 2021. The `BT_TOKEN` environment-variable auto-detection (which previously threw a hard error) is removed. Apps that still set `BT_TOKEN` for unrelated reasons will no longer hit the throw — pick a supported provider via the `publish` configuration instead.
...
The PowerShell `Invoke-TrustedSigning` path is now the **legacy fallback** (triggered when `toolsets.winCodeSign` is unset or below `"1.3.0"` — the v27 default `"1.1.0"` still uses this path). Set `toolsets.winCodeSign: "1.3.0"` to activate the faster `signtool /dlib` path, which reads a `metadata.json` file — that is what `additionalMetadata` feeds into.
...
// After
{
  "win": {
    ...
    // Optional: set to "1.3.0" to use signtool /dlib instead of the legacy PowerShell fallback
  },
  "toolsets": {
    "winCodeSign": "1.3.0"
  }
...
|---------|-------------|-------------|-----------------|
| `winCodeSign` | `0.0.0` (winCodeSign 2.6.0) | `"1.1.0"` | Windows Kits 10.0.26100.0; modern `signtool.exe` and `osslsigncode`. Set `"1.3.0"` for Azure Trusted Signing `signtool /dlib` |
...
For Azure Trusted Signing with the faster `signtool /dlib` path (recommended for `win.sign: { type: "azure" }`), pin `winCodeSign` to at least `"1.3.0"`:
...
Without this explicit pin the legacy PowerShell `Invoke-TrustedSigning` fallback is used, which requires the `TrustedSigning` PS module to be installed in the signing environment.
...
`PlatformPackager` exposed a public `info: Packager` field in v26 giving outside callers access to the whole-build orchestrator. **In v27 this field is `protected`** — external `packager.info.X` access is a compile error:

```ts
// v27 — compile error (TS2445: Property 'info' is protected …)
const tmpDir = packager.info.tempDirManager
await packager.info.emitArtifactCreated(event)
```

All commonly-needed properties have been promoted to public getters and methods directly on `PlatformPackager`. Migrate by dropping the `.info.` chain:

| Before (v26 public) | After (v27 public) |
|---|---|
...
The getters already on `PlatformPackager` in v26 are unchanged (`config`, `projectDir`, `buildResourcesDir`, `packagerOptions`, `appInfo`, `debugLogger`). Subclasses of `PlatformPackager` (the supported extension point for custom targets) retain access to `info` via inheritance — only outside-the-hierarchy callers need to migrate.
...
- [ ] ✓ `electronUpdaterCompatibility` (root / `mac` / `win` / `linux`) removed (auto-detected from `dependencies['electron-updater']`)
...
- [ ] `electron-updater` pinned to >= 4.0.0 (the legacy `update.yml` compat path will be removed in a future major release; v27 still emits it for older pins with a warning)
- [ ] `UpdateCheckResult.versionInfo` → `updateInfo` (electron-updater consumers; alias was deprecated since 4.0)
- [ ] `BT_TOKEN` env var dropped from CI if it was set for legacy Bintray auto-detect (no replacement; Bintray was sunset 2021)
...
- [ ] Replace external `packager.info.X` chains with the direct public getters / methods on `PlatformPackager` (`info` is now `protected` — subclasses still inherit access) (if applicable)
...
### `PlatformPackager.info` visibility change

The public `info: Packager` field created a hard coupling: any internal `Packager` refactor became a breaking change for every plugin that reached through `.info.`. v27 promotes the commonly-needed properties to direct public getters/methods on `PlatformPackager` (one stable surface) and tightens `info` itself to `protected` so subclasses (the supported extension point for custom targets) still have full access while outside callers don't.
