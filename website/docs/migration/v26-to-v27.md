...
| Azure extra sign keys | `"sign": { "ExcludeCredentials": "X" }` (in azureSignOptions) | `"sign": { "additionalMetadata": { "ExcludeCredentials": "X" } }` |
...
The PowerShell `Invoke-TrustedSigning` path is now the **legacy fallback** (triggered when `toolsets.winCodeSign` is unset or `"0.0.0"`). Set `toolsets.winCodeSign: "1.3.0"` to activate the faster `signtool /dlib` path, which reads a `metadata.json` file — that is what `additionalMetadata` feeds into.
