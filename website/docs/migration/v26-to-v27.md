...
- [ ] [Remove `squirrelWindows.customSquirrelVendorDir`](./v27-breaking-changes#squirrelwindowscustomsquirrelvendordir) — the option is deleted; point at a custom Squirrel toolset with the `ELECTRON_BUILDER_SQUIRREL_TOOLSET_DIR` env var instead (note the different directory layout).
