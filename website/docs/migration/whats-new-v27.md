...
| **Toolsets resolve to `"latest"`** | Unset / `"latest"` now fetch the newest bundle (winCodeSign 1.3.0, FUSE3 AppImage, NSIS 3.12); pin `"0.0.0"` to restore a legacy bundle. | [→](./v27-breaking-changes.md#toolset-defaults-resolve-to-latest-newest-bundle) |
| **Wine is the host install** | `toolsets.wine` defaults to `"system"`: building Windows targets on macOS now needs Wine installed (`brew install --cask wine-stable`). | [→](./v27-breaking-changes.md#toolset-defaults-resolve-to-latest-newest-bundle) |
| **macOS default entitlements tightened** | The bundled default grants only `allow-jit`; apps loading native modules or frameworks signed by another Team ID (or unsigned) must add `disable-library-validation` back in `build/entitlements.mac.plist`. | [→](./v27-breaking-changes.md#macos-default-entitlements-tightened) |
| **Missing production dependencies fail the build** | An unresolvable production dependency is now an error instead of a warning; `allowMissingDependencies: true` restores warn-only. | [→](./v27-breaking-changes.md#allowmissingdependencies-now-fails-the-build) |
| **`extraFiles` / `extraResources` `to` validated** | An absolute `to`, or one escaping the output dir, now throws. | [→](./v27-breaking-changes.md#extrafiles--extraresources-destinations-are-validated) |
| **Windows `publisherName` validated** | A `win.sign.publisherName` that does not match the signing certificate now fails the build. | [→](./v27-breaking-changes.md#windows-publishername-is-validated-against-the-signing-certificate) |
| **Suffixed update channels expand** | With `generateUpdatesFilesForAllChannels`, a channel such as `beta-arm64` now also writes `alpha-arm64.yml`. | [→](./v27-breaking-changes.md#suffixed-update-channels-now-expand-to-lower-channels) |
