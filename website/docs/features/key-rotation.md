...
| Linux repository GPG key | The user's package-manager keyring | `apt` / `dnf` / `zypper`, when you sign your packages and your packaging enforces it |
...
- If `app-update.yml` has no `publisherName` at all, verification is skipped.
...
- **deb / rpm.** electron-updater installs downloaded `.deb` / `.rpm` packages with the package manager's signature checks bypassed (e.g. `apt --allow-unauthenticated`), because electron-builder does not sign these packages. Plain `dpkg` performs no signature verification at all. (A gate for these bypass flags, `AppUpdater.allowUnverifiedLinuxPackages`, exists in v27+ only.)

If you sign your packages with GPG and distribute them through a repository that enforces it, the trust anchor is the public key in the user's keyring (`/etc/apt/trusted.gpg.d`, `/etc/pki/rpm-gpg`, ...). That keyring is populated by your packaging, not by electron-builder, so rotation follows the standard distribution playbook:
...
- [Auto Update](./auto-update.md) — updater behavior and options
