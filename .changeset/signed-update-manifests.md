...
Note for v26: the Linux package-manager signature-bypass flags used when installing `.deb`/`.rpm` updates are
not gated on this release line (`AppUpdater.allowUnverifiedLinuxPackages` from #9990 is v27+ only); manifest
signing covers the update metadata and the `sha512` of the downloaded package.
