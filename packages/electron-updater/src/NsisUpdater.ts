      ...
      execFile("powershell.exe", [`Get-AuthenticodeSignature '${tempUpdateFile}' | ConvertTo-Json -Compress`], {maxBuffer: 4 * 1024000, timeout: 60 * 1000}, (error, stdout, stderr) => {
        if (error != null || stderr) {
          try {
            ...
            this._logger.warn(`Cannot execute ConvertTo-Json: ${err.message}. Ignoring signature validation due to unsupported powershell version. Please upgrade to powershell 3 or higher.`)
            resolve(null)
            return
          }
