 ...
 * primitive `elevate.exe` uses) and reports the outcome through its exit code:
...
export function buildElevatedInstallerScript(installerPath: string, args: ReadonlyArray<string>): string {
  ...
  return [
    "try {",
    `  Start-Process -FilePath '${escapePowerShellSingleQuoted(installerPath)}'${argumentListParameter} -Verb RunAs -ErrorAction Stop`,
    "} catch {",
