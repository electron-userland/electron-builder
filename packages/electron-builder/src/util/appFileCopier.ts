...
import { FileSet } from "./AppFileWalker"
import { copyFileOrData } from "./asarUtil"
import { AsyncTaskManager } from "./asyncTaskManager"

export async function copyAppFiles(fileSet: FileSet, unpackedDest: string, packager: Packager) {
  ...
  for (let i = 0, n = fileSet.files.length; i < n; i++) {
    ...
    const relativePath = file.replace(fileSet.src, unpackedDest)
    if (stat.isFile()) {
      ...
      if (!dirToCreateForUnpackedFiles.has(fileParent)) {
        ...
        await ensureDir(fileParent.replace(fileSet.src, unpackedDest))
      }
