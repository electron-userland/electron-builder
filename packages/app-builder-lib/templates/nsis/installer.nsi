      ...
      !ifndef ONE_CLICK
          ...
          ${andIf} ${Silent}
            ${ifNot} ${UAC_IsAdmin}
              ...
              # loosened this folder's ACL for the current user. Only actually elevate if
              # a live check shows it's still needed. Both must hold: registryAddInstallInfo
              # writes InstallLocation to SHELL_CONTEXT (HKLM here), and that WriteRegStr
              # fails silently when unelevated -- checking $INSTDIR alone would let this
