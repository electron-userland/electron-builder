...
export class LinuxTargetHelper {
  ...
  computeDesktopEntry(targetSpecificOptions: CommonLinuxOptions, exec?: string, extra?: Record<string, string>): Promise<string> {
    ...
    const rawDesktopName = packager.info.metadata.desktopName
    const wmClass = !isEmptyOrSpaces(rawDesktopName) ? rawDesktopName!.replace(/\.desktop$/, "") : appInfo.productName
