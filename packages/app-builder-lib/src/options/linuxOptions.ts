...
export interface LinuxConfiguration extends CommonLinuxOptions, PlatformSpecificBuildOptions {
  /**
   * Target package type: list of `AppImage`, `flatpak`, `snap`, `deb`, `rpm`, `freebsd`, `pacman`, `p5p`, `apk`, `gentoo`, `7z`, `zip`, `tar.xz`, `tar.lz`, `tar.gz`, `tar.bz2`, `dir`.
   *
...
/**
 * Emits a single self-contained `.ebuild`, published like all other artifacts, plus the app icon as a
 * small release asset beside it. The ebuild references the archive the project already publishes, so a
 * `tar.gz`, `tar.bz2`, `tar.xz` or `tar.lz` target must be enabled alongside `gentoo`. Whoever installs
 * it places it in an overlay and runs `ebuild <file> manifest`, which fetches the archive and computes
 * its digest; no Manifest is shipped, so nothing can go stale between builds.
 *
 * The desktop entry and launcher are generated by the ebuild itself.
 */
export interface GentooOptions extends CommonLinuxOptions, TargetSpecificOptions {
  /**
   * The package name (`PN`). Defaults to the application name with a `-bin` suffix, which is the Gentoo
   * convention for a package that installs prebuilt binaries rather than compiling from source.
   */
  readonly packageName?: string | null
  ...
  /**
   * Whether the generated ebuild's `src_prepare` drops `node_modules/*\/prebuilds/` builds for other
   * platforms and for the other libc. Native modules packaged with prebuildify ship a binary per
   * platform and per libc; only one can load on the merging machine, and the rest leave Portage
   * reporting unresolved sonames. This runs in the ebuild, on the user's machine; the published
   * archive is not modified.
   * @default true
   */
  readonly pruneForeignPrebuilds?: boolean | null

  /**
   * The `RDEPEND` atoms of the generated ebuild. Use the `"default"` keyword to extend the default list
   * instead of replacing it.
   *
   * Defaults to the shared libraries the Electron binary links against.
   */
  readonly depends?: Array<string> | null

  /**
   * The `KEYWORDS` of the generated ebuild. Defaults to `-*` plus a testing keyword (`~amd64`, `~arm64`, …)
   * for each architecture that was built.
   */
  readonly keywords?: Array<string> | null

  /**
   * The `EAPI` of the generated ebuild.
   * @default 8
   */
  readonly eapi?: number | null

  /**
   * Where the ebuild gets the app icon from, as a second `SRC_URI` entry.
   *
   * - `"repository"`: the icon file inside the source repository at the release tag, through the detected
   *   forge's raw-file URL. Nothing to upload, and it works for releases that are already published. The
   *   icon must live inside the project directory.
   * - `"asset"`: the icon is written to the output directory as `<PN>-<version>.<ext>`, to be uploaded
   *   beside the archive. Works with any `distUrl`.
   * - `false`: no icon; the desktop entry shows a generic one.
   *
   * When the chosen source cannot provide the icon, the build warns and the ebuild is emitted without one.
   * @default repository
   */
  readonly iconSource?: "repository" | "asset" | false | null

  /**
   * The kind of git hosting the `repository` field points at, used to derive the release download URL.
   * `github.com`, `gitlab.com`, `codeberg.org`, `bitbucket.org` and `git.sr.ht` are recognised by name
   * and any other host is asked (see `probeForge`), so this is only needed for a self-hosted instance
   * that cannot be probed. `gitea` also covers Forgejo and Codeberg.
   */
  readonly forge?: "github" | "gitlab" | "gitea" | null

  /**
   * Whether an unrecognised git host is asked which forge it is, with one unauthenticated request to
   * its API (Gitea and Forgejo answer `/api/v1/version`; GitLab is identified by `/api/v4/version`).
   * Best effort with a five second timeout; the build never fails because of it.
   * @default true
   */
  readonly probeForge?: boolean | null

  /**
   * The base URL the archive and icon will be downloadable from; their file names are appended to it.
   *
   * Defaults to the URL of the configured `publish` provider (GitHub, generic, S3, Spaces, R2), or
   * failing that to the release download URL of the detected forge for the `repository` field of
   * `package.json`, at tag `v<version>`. Required when neither can be determined.
   */
  readonly distUrl?: string | null

  /**
   * The `LICENSE` of the generated ebuild, which must be a name from Gentoo's `licenses/` directory.
   * Defaults to the `license` field of `package.json`.
   */
  readonly license?: string | null
}
