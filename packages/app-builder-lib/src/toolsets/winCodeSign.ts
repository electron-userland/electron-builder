...
export async function getSignToolPath(winCodeSign: ToolsetConfig["winCodeSign"], isWin: boolean, resourcesDir: string): Promise<ToolInfo> {
  ...
  if (isWin) {
    ...
    return { path: await _getWindowsSignToolExe({ winCodeSign, arch: signtoolArch, resourcesDir }) }
  } else {
    const vendor = await _getOsslSigncodeBundle(winCodeSign, resourcesDir)
...
export async function getWindowsKitsBundle({
  ...
  if (typeof resolved === "object") {
    const vendorPath = await getCustomToolsetPath(resolved, resourcesDir)
...
async function _getWindowsSignToolExe({ winCodeSign, arch, resourcesDir }: { winCodeSign: ToolsetConfig["winCodeSign"]; arch: Arch; resourcesDir: string }) {
  const resolved = winCodeSign ?? "1.1.0"
  if (resolved === "0.0.0") {
    const vendorPath = await getLegacyWinCodeSignBin()
    if (isOldWin6()) {
  ...
  const vendorPath = await getWindowsKitsBundle({ winCodeSign: resolved, arch, resourcesDir })
...
export async function getRceditBundle(winCodeSign: ToolsetConfig["winCodeSign"], resourcesDir: string): Promise<{ x86: string; x64: string }> {
  const x86 = "rcedit-x86.exe"
  const x64 = "rcedit-x64.exe"
  const resolved = winCodeSign ?? "1.1.0"
  if (resolved === "0.0.0") {
    const vendorPath = await getLegacyWinCodeSignBin()
    return { x86: path.resolve(vendorPath, "rcedit-ia32.exe"), x64: path.resolve(vendorPath, x64) }
  }
  if (typeof resolved === "object") {
    const vendorPath = await getCustomToolsetPath(resolved, resourcesDir)
