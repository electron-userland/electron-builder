...
import asar, { Metadata } from 'asar'
// import * asarHelper from 'asar/lib/filesystem'
...
//   constructor(readonly src: string, readonly header: Metadata, readonly headerSize: number = -1) {
//     new asarHelper.FileSystem
//   }
...
export async function readAsarHeader(archive: string) {
...
export async function readAsar(archive: string) {
  // const { header, size } = await readAsarHeader(archive)
  // return new AsarFilesystem(archive, JSON.parse(header), size)
}

export async function readAsarJson(archive: string, file: string) {
  const buffer = await asar.extractFile(archive, file)
